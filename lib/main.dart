...
class _PaginaInicialState extends State<PaginaInicial> {
  static const _itensKey = 'itens_compra';
  static const _compradosKey = 'itens_comprados';
  ...
  Future<void> _carregarDados() async {
    ...
    final List<String> itensSalvos =
        List<String>.from(prefs.getStringList(_itensKey) ?? []);
    final List<String> statusSalvos =
        List<String>.from(prefs.getStringList(_compradosKey) ?? []);
    final List<bool> statusConvertidos =
        statusSalvos.map((valor) => valor == 'true').toList();

    if (statusConvertidos.length < itensSalvos.length) {
      statusConvertidos
          .addAll(List<bool>.filled(itensSalvos.length - statusConvertidos.length, false));
    } else if (statusConvertidos.length > itensSalvos.length) {
      statusConvertidos.removeRange(itensSalvos.length, statusConvertidos.length);
    }
    ...
    setState(() {
      itensCompra = itensSalvos;
      itensComprados = statusConvertidos;
    });
  }

  Future<void> _salvarDados() async {
    final prefs = _prefs ??= await SharedPreferences.getInstance();
    await prefs.setStringList(_itensKey, List<String>.from(itensCompra));
    await prefs.setStringList(
      _compradosKey,
      itensComprados.map((comprado) => comprado.toString()).toList(),
    );
  }
